...
def signin(request):
    ...
    if 'email' not in request_body or 'password' not in request_body:
        return JsonResponse({'message': 'All fiels required'}, status=status.HTTP_412_PRECONDITION_FAILED)

    # Check if email exist in DB
    user = User.objects.all().filter(email__exact=request_body.get("email"))
    if not user.exists():
        return JsonResponse({'success': False, 'message': 'Unknow email'}, status=status.HTTP_412_PRECONDITION_FAILED)

    # Check if password match the given email
    if request_body.get('password') == user.values()[0]['password']:
